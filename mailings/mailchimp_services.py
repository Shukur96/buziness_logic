...
def _add_email_to_mailchimp_audience(audience_id: str, email:str) -> None:
    # Добовляет email в  MailChimp аудиторию с идентификатором audiens_id
...
def _get_mailchimp_subscriber_hash(email: str) -> Optional[str]:
    #Возвращает идентификатор емайла в Маилчимп или None если емаил там не найден
    
    members = _get_mailchimp_client() \
        .search_members \
        .get(query = email,
            fields ='exact_matches.members.id' ) \
        .get('exect_matches').get('members')
...
def _add_mailchimp_tag(audience_id: str, subscriber_hash: str,tag : str) -> None:
    #Добовляет tag для email'а с ивентификатором  subscriber_hash в аулиторий audience_id
